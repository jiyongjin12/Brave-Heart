...
public class InventoryDragDropSystem : MonoBehaviour
{
    ...
    [SerializeField] private List<Inventory> inventoryTetrisList;

    private Inventory draggingInventoryTetris;
    ...
    private void Start()
    {
        foreach (Inventory inventoryTetris in inventoryTetrisList)
        {
    ...
    public void StartedDragging(Inventory inventoryTetris, PlacedObject placedObject)
    {
        // Started Dragging
    ...
    public void StoppedDragging(Inventory fromInventoryTetris, PlacedObject placedObject)
    {
        ...
        Inventory toInventoryTetris = null;
        ...
        if (toInventoryTetris != null)
        {
            ...
            bool tryPlaceItem = toInventoryTetris.TryPlaceItem(placedObject.GetPlacedObjectTypeSO() as ItemSO, placedObjectOrigin, dir);
            ...
            {
                // 아이템 위에 아이템 놨을시
                TooltipCanvas.ShowTooltip_Static("Test!");
                FunctionTimer.Create(() => { TooltipCanvas.HideTooltip_Static(); }, .7f, "HideTooltip", true, true);

                // Drop on original position
                fromInventoryTetris.TryPlaceItem(placedObject.GetPlacedObjectTypeSO() as ItemSO, placedObject.GetGridPosition(), placedObject.GetDir());
            }
        }
        else
        {
